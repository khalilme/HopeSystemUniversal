...
namespace WCFServiceWebRole.MyClasses
{
    class Analyzer
    {
        ...
        public static SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["HaifaDBconnstr"].ConnectionString);
        ...
        public static List<ArabicWord> ExtractArabicWords(string EntireText)
        {
            ...
            for (int i = 0; i < EntireText.Length; i++)
            {
                ...
                // Amal sh
                // unicode of # is 35 
                // ادا الكلمة تبدأ ب # لا تحذف #
                if ((Unicode >= 1569 && Unicode <= 1618) || (Unicode == 35)) //إذا كان الحرف من الحروف العربية المعتادة
                {
        ...
        private static List<WordInfo> ProcessWord(ArabicWord WordToProcess)
        {
            ...
            ////    Amal 22
            //لو الكلمة بتبدأ ب # لا تفحصها
            if (WordToProcess.word.StartsWith("#"))
            {
                //     WordToProcess.word = "#" + WordToProcess.word;
